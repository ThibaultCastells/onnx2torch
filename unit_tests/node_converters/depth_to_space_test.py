...
def _test_depth_to_space(
    ...
    model = make_model_from_nodes(
        nodes=node, initializers={}, inputs_example=test_inputs, opset_version=opset
    )
...
def test_depth_to_space(input_shape: List[int], blocksize: int, opset: int) -> None:
    _test_depth_to_space(
        input_shape=input_shape, blocksize=blocksize, mode="CRD", opset=opset
    )
