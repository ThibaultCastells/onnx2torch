...
def _test_conv(
    ...
    **kwargs,
    ...
    weights = np.random.uniform(low=-1.0, high=1.0, size=weights_shape).astype(
        np.float32
    )
...
def test_conv2d_base_params() -> None:  # pylint: disable=missing-function-docstring
    ...
    all_variants = product(
        op_type_variants,
        in_channels_variants,
        out_channels_variants,
        input_hw_variants,
...
def test_conv_transpose_output_pads() -> None:  # pylint: disable=missing-function-docstring
    ...
    all_variants = product(
        input_hw_variants, stride_variants, dilations_variants, output_pads_variants
    )
