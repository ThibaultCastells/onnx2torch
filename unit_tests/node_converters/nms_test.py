...
def _test_nms(
    ...
    if max_output_boxes_per_class is not None:
        test_inputs["max_output_boxes_per_class"] = np.array(
            max_output_boxes_per_class, dtype=np.int64
        )
...
_BOXES_CXCYWH_FORMAT_TEST = np.array(
    [[[1.0, 1.0, 1.1, 1.1], [1.5, 1.5, 1.6, 1.6]]], dtype=np.float32
)
...
_SCORES_FLIPPED_COORDINATES_TEST = np.array(
    [[[0.9, 0.75, 0.6, 0.95, 0.5, 0.3]]], dtype=np.float32
)
...
@pytest.mark.parametrize(
    ...
    (
        (
            _BOXES_CXCYWH_FORMAT_TEST,
            _SCORES_CXCYWH_FORMAT_TEST,
            3,
            0.1,
            0.0,
            1,
        ),  # center point box format
        # flipped coordinates
        # (_BOXES_FLIPPED_COORDINATES_TEST, _SCORES_FLIPPED_COORDINATES_TEST, 3, 0.5, 0.0, None),
        (
            _BOXES_IDENTICAL_BOXES_TEST,
            _SCORES_IDENTICAL_BOXES_TEST,
            3,
            0.5,
            0.0,
            None,
        ...
        (
            _BOXES_IOU_SCORE_TEST,
            _SCORES_IOU_SCORE_TEST,
            3,
            0.5,
            0.4,
            None,
        ),  # suppress by IOU and score
        (
            _BOXES_2_BATCHES_TEST,
            _SCORES_2_BATCHES_TEST,
            2,
            0.5,
            0.0,
            None,
        ...
        (
            _BOXES_2_CLASSES_TEST,
            _SCORES_2_CLASSES_TEST,
            2,
            0.5,
            0.0,
            None,
