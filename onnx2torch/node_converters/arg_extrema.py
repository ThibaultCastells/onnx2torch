...
class OnnxArgExtremum(nn.Module, OnnxToTorchModule):
    def __init__(
        self, operation_type: str, axis: int, keepdims: int, select_last_index: int
    ):
        super().__init__()
        self.axis = axis
        self.keepdims = bool(keepdims)
    ...
    def forward(self, data: torch.Tensor) -> torch.Tensor:
        if self.select_last_index:
            ...
            extremum_index_flipped = self.extremum_function(
                flipped, dim=self.axis, keepdim=self.keepdims
            )
            extremum_index_original = (
                data.size(dim=self.axis) - 1 - extremum_index_flipped
            )
...
def _(node: OnnxNode, graph: OnnxGraph) -> OperationConverterResult:
    ...
    return OperationConverterResult(
        torch_module=OnnxArgExtremum(
            ...
            select_last_index=node.attributes.get(
                "select_last_index", DEFAULT_SELECT_LAST_INDEX
            ),
        ),
