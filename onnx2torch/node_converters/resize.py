...
def _get_torch_align_corners(
    mode: str, coordinate_transformation_mode: str
) -> Optional[bool]:
    if mode == "nearest":
        return None
...
def _onnx_mode_to_torch_mode(onnx_mode: str, dim_size: int) -> str:
    ...
    if torch_mode is None:
        raise NotImplementedError(
            f'{dim_size}D input is not implemented for "{onnx_mode}" mode.'
        )
    ...
    def forward(  # pylint: disable=missing-function-docstring
        ...
        if sizes is not None:
            if sizes.nelement() != 0:
                ...
                if not self.ignore_bs_ch_size and input_shape[:2] != sizes[:2]:
                    raise NotImplementedError(
                        "Pytorch's interpolate cannot resize channel or batch dimensions."
                    )
                sizes = sizes[2:]
            else:
                sizes = None
        ...
        if scales is not None:
            if scales.nelement() != 0:
                ...
                if scales[:2] != [1, 1]:
                    raise NotImplementedError(
                        "Pytorch's interpolate cannot scale channel or batch dimensions."
                    )
                scales = scales[2:]
            else:
                scales = None
...
def _(node: OnnxNode, graph: OnnxGraph) -> OperationConverterResult:  # pylint: disable=unused-argument
    node_attributes = node.attributes
    coordinate_transformation_mode = node_attributes.get(
