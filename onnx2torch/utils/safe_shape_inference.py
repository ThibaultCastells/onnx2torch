...
def _shape_inference_by_model_path(
    ...
    **kwargs,
    ...
    inferred = onnx.load(output_path)
    base_dir = Path(output_path).parent
    try:
        external_data_helper.load_external_data_for_model(inferred, base_dir)
        external_data_helper.convert_model_from_external_data(inferred)
    except FileNotFoundError:
        # Temporary inference outputs may not emit external data; ignore missing files.
        pass
